...
- Plans (2026-09-26): navbar (হোম · আমার পোস্টার · প্ল্যান), `/plans` page, dashboard template filter. The paid plan is renamed **premium → pro** (৳100/month, display only until payments exist): code, API, tests and `REVIEWER_PRO_PHONE` (old `REVIEWER_PREMIUM_PHONE` still read). Stored users: `db:migrate-plan-pro` (run on the local DB; Atlas after deploying); until then the API reads `premium` as pro. Mentions of premium elsewhere in this plan are historical
- Premium plan (2026-09-26): third plan `premium`, ৳150/month, unlimited posters/regenerations/headlines (limit `null` = counted, never blocked; per-minute abuse limits stay), no watermark. The old 'premium' → pro fallback was removed, so `db:migrate-plan-pro` must run on Atlas **before** this API deploys (otherwise the pro test account stored as `premium` becomes unlimited)
